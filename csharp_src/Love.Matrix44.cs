...
namespace Love
{
    ...
    {
        public LinkedList<Matrix4x4?> list = new LinkedList<Matrix4x4?>();
        ...
        public void Push()
        {
            list.AddLast(Matrix4x4.Identity);
        }

        public void Scale(float sx, float sy)
        {
            var m = list.Last.Value.Value;
            list.AddLast(m * Matrix4x4.CreateScale(sx, sy, 1));
        }

        public void Translate(float ox, float oy)
        {
            var m = list.Last.Value.Value;
            list.AddLast(m * Matrix4x4.CreateTranslation(ox, oy, 0));
        }

        public void Rotate(float r)
        {
            var m = list.Last.Value;
            list.AddLast(m * Matrix4x4.CreateRotationZ(r));
        }

        public Vector2 TransformPoint(Vector2 p)
        {
            var v3 = Matrix4x4.TransformVector3(new Vector3(p, 0), list.Last.Value.Value);
            return new Vector2(v3.x, v3.y); 
        }

        public Vector2 TransformPointInvert(Vector2 p)
        {
            Matrix4x4.Invert(list.Last.Value.Value, out var result);
            var v3 = Matrix4x4.TransformVector3(new Vector3(p, 0), result);
            return new Vector2(v3.x, v3.y);
        }
    }
    /// <summary>
